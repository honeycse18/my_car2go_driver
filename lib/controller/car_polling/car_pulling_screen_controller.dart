...
class RequestCarPullingScreenController extends GetxController {
  ...
  Rx<ShareRideRequestsStatus> shareRideTypeTab =
      ShareRideRequestsStatus.findRide.obs;
  Rx<ShareRideActions> selectedActionForRideShare =
      ShareRideActions.unknown.obs;
  Rx<ShareRideRequestsStatus> selectedShareRideStatus =
      ShareRideRequestsStatus.unknown.obs;
  PagingController<int, ShareRideHistoryDoc> shareRideHistoryPagingController =
      PagingController(firstPageKey: 1);

  void onShareRideTabTap(ShareRideRequestsStatus value) {
    shareRideTypeTab.value = value;
    if (value == ShareRideRequestsStatus.findRide) {
      selectedActionForRideShare.value = ShareRideActions.myRequest;
      selectedShareRideStatus.value = ShareRideRequestsStatus.unknown;
    } else if (value == ShareRideRequestsStatus.offering) {
