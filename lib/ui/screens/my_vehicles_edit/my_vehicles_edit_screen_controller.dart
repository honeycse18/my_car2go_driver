...
class MyVehicleEditScreenController extends GetxController {
  final profileService = Get.find<ProfileService>();
  final myVehiclesController = Get.find<MyVehiclesScreenController>();
  ...
  void onUpdateVehicleTap() async {
    if (validateVehicleRegister() == false) {
      return;
    }
    final request = <String, dynamic>{
      '_id': myVehicleDetails.id,
      'driver_information': {
    ...
    isLoading = true;
    final response = await APIRepo.updateVehicle(request);
    isLoading = false;
    if (response == null) {
      AppDialogs.showErrorDialog(
...
/*   void selectGearType(String gearType) {
  ...
  Future<void> _fillVehicleDetails(MyVehicleDetails vehicle) async {
    selectedSeatCount = vehicle.seat;
    selectedYear =
        int.tryParse(vehicle.year) ?? AppConstants.defaultUnsetDateTimeYear;
    yearController.text = vehicle.year;
    numberPlateTextEditingController.text = vehicle.carNumberPlate;
    vehicleImages = vehicle.images;
    dynamicFieldValues = _setDynamicFieldValuesFromProfile(
        dynamicFieldValues: dynamicFieldValues,
        profileDynamicFieldValues: vehicle.dynamicFields);
    dynamicFieldValues = addDynamicFieldValuesNonExistingFromProfile(
        dynamicFieldValues: dynamicFieldValues,
        profileDynamicFieldValues: vehicle.dynamicFields);
    await _getVehicleCategoryList();
    final foundCategory = vehicleCategories
        .firstWhereOrNull((category) => category.id == vehicle.vehicleType.id);
    selectedVehicleCategory = foundCategory ?? VehicleTypeInfo.empty();
    await _getVehicleBrands(selectedVehicleTypeID: selectedVehicleCategory.id);
    selectedVehicleBrand =
        vehicleBrands.firstWhereOrNull((brand) => brand.name == vehicle.brand);
    if (selectedVehicleBrand != null) {
      await _getVehicleModels(
          selectedVehicleTypeID: selectedVehicleCategory.id,
          selectedBrandID: selectedVehicleBrand!.id);
      selectedVehicleModel = vehicleModels
          .firstWhereOrNull((model) => model.name == vehicle.model);
    }
    update();
  }
  ...
  Future<void> _getMyVehicleDetails() async {
    isLoading = true;
    final response =
    ...
    update();
    _fillVehicleDetails(myVehicleDetails);
  }
