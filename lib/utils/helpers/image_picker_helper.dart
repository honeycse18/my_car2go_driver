...
class ImagePickerHelper {
  ...
  static Future<Uint8List?> convertImageToJPG(image_picker.XFile image,
      {int qualityAsPercentage = 80}) async {
    try {
      final selectedImageBytesBefore = await image.readAsBytes();
      final img.Image? capturedImage =
          img.decodeImage(selectedImageBytesBefore);
      if (capturedImage == null) return null;
      dev.log(
          'Image size before: ${selectedImageBytesBefore.lengthInBytes} bytes');
      final resizedJPGImage =
          img.encodeJpg(capturedImage, quality: qualityAsPercentage);
      capturedImage;
      dev.log('Image byte size after: ${resizedJPGImage.lengthInBytes}');
      return resizedJPGImage;
    } catch (e) {
      return null;
    }
  }
  ...
  static Future<List<Uint8List?>> getProcessedImages(
    ...
    final List<Uint8List?> processedImages =
      ...
      return await convertImageToJPG(pickedImage);
    }).toList());
    // final List<Uint8List> filteredProcessedImages = processedImages.whereNotNull().toList();
  ...
  static Future<Uint8List?> getProcessedImage(
    ...
    final Uint8List? processedImage =
        ...
        await convertImageToJPG(pickedImage);
